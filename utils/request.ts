...
export default function request<R>(url: string, init: Options = {}): Promise<R> {
  const { query, notify = false, baseURL = process.env.NEXT_PUBLIC_LOCAL_HOST, onUnauthorized, ...options } = init;
  ...
  if (options.body instanceof FormData) {
    if (isBrowser) {
      ...
      const boundary = options.body.getBoundary();
      ...
      return parseResponse<R>(response).then(
        ({ code, msg, payload }) => {
          ...
          if (isStatusOk(status) && code === 200) {
            notify && isBrowser && toast.success(msg);
